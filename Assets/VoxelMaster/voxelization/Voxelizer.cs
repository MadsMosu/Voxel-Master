...
public class Voxelizer : EditorWindow {
    ...
    private float GetVoxelDensity (Vector3 voxelPos, TriangleRegions regions, MathToolBox.Triangle triangle, Bounds S, float dist) {
        ...
        if (intersectA) {
            if (eDist >= 0 && fDist >= 0 && gDist >= 0) return R1Density (regions.a, aDist); // R1
            else if (dDist >= 0 && dDist <= 1 && gDist < 0) return R234Density (regions.a, regions.g, aDist, gDist); // R2
            else if (cDist >= 0 && cDist <= 1 && fDist < 0) return R234Density (regions.a, regions.f, aDist, fDist); // R3
            else if (bDist >= 0 && bDist <= 1 && eDist < 0) return R234Density (regions.a, regions.e, aDist, eDist); // R4
    ...
    private float R1Density (MathToolBox.Plane3D a, float aDist) {
        return 1 - (Mathf.Abs (aDist) / filterWidth);
    }

    private float R234Density (MathToolBox.Plane3D a, MathToolBox.Plane3D b, float aDist, float bDist) {
