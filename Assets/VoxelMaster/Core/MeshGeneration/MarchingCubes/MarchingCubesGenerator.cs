...
public class MarchingCubes : VoxelMeshGenerator {
    public override MeshData GenerateMesh (VoxelChunk chunk, Func<Vector3, float> densityFunction) {
        int numCells = chunk.neighborFieldSize.x * chunk.neighborFieldSize.y * chunk.neighborFieldSize.z;
        var vertices = new List<Vector3> (5 * numCells * 3);
        var triangleIndices = new List<int> (5 * numCells * 3);
        int triangleIndex = 0;

        chunk.TraverseWithNeighbors (delegate (int x, int y, int z, Voxel v) {
            if (v.Equals (default (Voxel))) return;
            var cellPos = new Vector3Int (x, y, z);
            if (cellPos.x + 1 >= chunk.neighborFieldSize.x || cellPos.y + 1 >= chunk.neighborFieldSize.y || cellPos.z + 1 >= chunk.neighborFieldSize.z) return;

            float[] cubeDensity = new float[8] {
                chunk.GetVoxel (cellPos + Lookup.cubeVertOffsets[0]).density,
                chunk.GetVoxel (cellPos + Lookup.cubeVertOffsets[1]).density,
                chunk.GetVoxel (cellPos + Lookup.cubeVertOffsets[2]).density,
                chunk.GetVoxel (cellPos + Lookup.cubeVertOffsets[3]).density,
                chunk.GetVoxel (cellPos + Lookup.cubeVertOffsets[4]).density,
                chunk.GetVoxel (cellPos + Lookup.cubeVertOffsets[5]).density,
                chunk.GetVoxel (cellPos + Lookup.cubeVertOffsets[6]).density,
                chunk.GetVoxel (cellPos + Lookup.cubeVertOffsets[7]).density,
            };
