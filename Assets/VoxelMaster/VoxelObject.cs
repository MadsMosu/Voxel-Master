    ...
    public class VoxelObject : MonoBehaviour {
        ...
        void Start() {
            ...
            if (original) {
                ...
                GenerateSDF(transform.position, center, radius / 2);
        ...
        private void Islands() {
                ...
                for (int nx = -1; nx <= 1; nx++)
                    for (int ny = -1; ny <= 1; ny++)
                        for (int nz = -1; nz <= 1; nz++) {
                            ...
                            var neihbourLabel = labels[Util.Map3DTo1D(new Vector3Int(x + nx, y + ny, z + nz), chunkSize)];
                            if (neihbourLabel > 0) neighborLabels.Add(neihbourLabel);
            ...
            for (int i = 0; i < labels.Length; i++) {
                ...
                var coord = Util.Map1DTo3D(i, chunkSize);
        ...
        private void OnDrawGizmosSelected() {
            ...
            chunk.voxels.Traverse((x, y, z, v) => {
                ...
                Gizmos.DrawWireSphere(transform.TransformPoint(new Vector3(x, y, z)), .25f);
            });
        }
